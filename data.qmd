...
::: {.callout-warning}
As detailed above, `read_csv()` is how we will import CSV data files into R. This section details an alternative way to import CSV data files, but it will not produce the same kind of variable.  Using RStudio's built-in "Import Dataset" functionality will produce dataframes, whereas `read_csv` produces "tibbles". Further information about the difference between plain dataframes and tibbles can be found in @sec-dataframes.
:::
...
Note, however, that this variable looks a bit different from the `sales` variable we created using `read_csv`. Again, the differences will be discussed more in @sec-dataframes.
...
- Download data
    ...
    - https://doi.org/10.3886/ICPSR36797.v1
...
- Unzip the data and associated files and place them in the "data" directory/folder you just created
- Briefly review the codebook