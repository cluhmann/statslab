...
Observe that the first line of the output reads ``# A tibble: `r nba %>% nrow() %>% comma()` x `r nba %>% ncol()` ``. This is an example of meta-data, in this case the number of observations/rows and variables/columns in `nba`. The actual data itself are the subsequent table of values. Now let's pipe the `nba` data frame into `group_by(college)`:
...
Observe that now there is additional meta-data: `# Groups:   college [345]` indicating that the grouping structure meta-data has been set based on the unique values of the variable `college`. On the other hand, observe that the data has not changed: it is still a table of `r nba %>% nrow() %>% comma()` $\times$ `r nba %>% ncol()` values.
...
Observe that there are now `r by_college_annually %>% nrow()` rows to `by_origin_monthly` because there are `r nba %>% ndistict(college)` unique colleges. 
