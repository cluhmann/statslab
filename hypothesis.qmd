...
Let's reconsider the question we asked in @sec-ht-case-study: is Michael Jordan better than Steph Curry? We'll again investigate if, on average, Michael Jordan or Steph Curry averaged more points per game.

Let's revisit the null distribution for our test.  We can construct the distribution for the test statistic $\bar{x}_{MJ} - \bar{x}_{SC}$ as we did in @sec-ht-case-study.
...
We can then visualize how these two compare @fig-t-stat-3.
